#pragma FXC interface_arrays ps_5_0 main
...
iChangeColour gAbstractColourChanger[2];
iChangeColour gAbstractColourChangerB;
iAlpha gAlphaChooser[3];

int alphaIndex;

float4 main( PS_INPUT Input ) : SV_TARGET
{
    float3 ModifiedColour = gAbstractColourChanger[1].ChangeColour(Input.Colour);
    ModifiedColour = gAbstractColourChangerB.ChangeColour(ModifiedColour);

    return float4(ModifiedColour,gAlphaChooser[alphaIndex].ChooseAlpha()); 
}
